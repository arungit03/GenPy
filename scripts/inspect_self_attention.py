"""Inspect single-head causal self-attention without building a GPT model."""
...
def main() -> int:
    """Parse arguments and inspect causal self-attention."""
    ...
    try:
        ...
        attention = CausalSelfAttention(
            embedding_dim=app_config.embeddings.embedding_dim,
            max_sequence_length=app_config.positional_encoding.max_sequence_length,
            dropout=args.dropout if args.dropout is not None else app_config.attention.dropout,
    ...
    logger.info("Self-attention inspection completed successfully.")
    _print_summary(attention, token_metadata.vocab_size, app_config.model.vocab_size, device)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Inspect GenPy LLM causal self-attention.")
...
def _print_summary(
    ...
    print("GenPy LLM Causal Self-Attention")
    print("===============================")
    ...
    print()
    print("This is single-head random self-attention; weights are not meaningful before training.")
...
def _print_batch_forward(
    ...
    print()
    print("Prepared batch self-attention pass")
    print("==================================")
    if len(dataset) == 0:
    ...
    if show_matrix:
        _print_attention_matrix(weights[0])
...
def _print_attention_matrix(matrix: torch.Tensor, max_rows: int = 8) -> None:
    shown = min(matrix.shape[0], max_rows)
    print()
    print("Attention matrix preview")
    print("========================")
...
def _report_error(exc: Exception, debug: bool) -> None:
    ...
    if debug:
        logger.exception("Self-attention inspection failed.")
    else:
